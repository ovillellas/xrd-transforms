# tests for capi implementation of angles_to_dvec
#
...
import pytest
import numpy as np
from xrd_transforms._new_transforms_capi import anglesToDVec as angles_to_dvec

Experiment = namedtuple('Experiment', ['angs', 'beam_vec', 'eta_vec', 'chi', 'rmat_c'])

@pytest.fixture(scope='module')
def experiment():
    '''just some simple arguments for angles_to_dvec that happen to be right
...
def test_correct(experiment):
    result = angles_to_dvec(experiment.angs,
                            experiment.beam_vec,
                            experiment.eta_vec,
                            experiment.chi,
                            experiment.rmat_c)
...
def test_incorrect_single_ang(experiment):
    ...
    with pytest.raises(ValueError):
        result = angles_to_dvec(experiment.angs[0],
                                experiment.beam_vec,
                                experiment.eta_vec,
                                experiment.chi,
                                experiment.rmat_c)
