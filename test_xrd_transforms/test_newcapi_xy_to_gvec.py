...
import pytest
import numpy as np
from xrd_transforms._new_transforms_capi import detectorXYToGvec as xy_to_gvec
...
def test_incorrect_single(experiment):
    # xy_det is not supported to be just one dimensional
    with pytest.raises(ValueError):
        result = xy_to_gvec(experiment.xy_det[0],
                            experiment.rmat_d,
                            experiment.rmat_s,
                            experiment.tvec_d,
                            experiment.tvec_s,
...
def test_incorrect_beam_vec(experiment):
    # bad number of dimensions (more than 1)
    with pytest.raises(ValueError):
        result = xy_to_gvec(experiment.xy_det,
                            experiment.rmat_d,
...
def test_incorrect_eta_vec(experiment):
    # bad number of dimensions (more than 1)
    with pytest.raises(ValueError):
        result = xy_to_gvec(experiment.xy_det,
                            experiment.rmat_d,
